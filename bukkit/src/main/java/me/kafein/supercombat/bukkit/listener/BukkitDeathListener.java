...
public class BukkitDeathListener implements Listener {
    ...
    public void onDeath(PlayerDeathEvent event) {
        ...
        if (ConfigKeys.Settings.DEATH_UNTAGGING_ENEMY.getValue()) {
            TagHandler.unTagPlayer(tag.getOtherUser().getUuid(), UntagReason.ENEMY_DEATH);
        }
        if (ConfigKeys.Settings.DEATH_UNTAGGING_SELF.getValue()) {
            TagHandler.unTagPlayer(uuid, UntagReason.SELF_DEATH);
        }
    ...
    public void onDeath(EntityDeathEvent event) {
        ...
        if (ConfigKeys.Settings.DEATH_UNTAGGING_ENEMY.getValue()) {
            TagHandler.unTagPlayer(attacker.getUniqueId(), UntagReason.ENEMY_DEATH);
        }
